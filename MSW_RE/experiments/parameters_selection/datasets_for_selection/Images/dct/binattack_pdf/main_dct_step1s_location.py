...
class ImageAnnotator:
    ...
    def load_saved_points(self):
        json_path = os.path.join(self.save_dir, "all_points.json")
        if os.path.exists(json_path):
            try:
                with open(json_path, "r", encoding='utf-8') as f:
                    data = json.load(f)
                    if isinstance(data, list):
                        self.points_list = data
                        self.current_index = len(data)
                        print(f"✅ 已加载 {len(data)} 张图像的标注点，将从第 {self.current_index + 1} 张图开始继续标注。")
                    else:
                        print("⚠️ JSON 格式错误，未加载已有标注点。")
            except Exception as e:
                print(f"⚠️ 加载 all_points.json 失败: {e}")
...
#     else:
#         annotator = ImageAnnotator(image_paths, save_dir=folder_selected)
#         cv2.waitKey(0)
...
json_file = "scanned_org/all_points.json"
...
image_paths = sorted([
    f for f in os.listdir('scanned_org') if f.lower().endswith(".bmp")
])
print(len(image_paths), len(source_points_list))
for idx in range(len(image_paths)):
    image = cv2.imread(os.path.join("scanned_org", image_paths[idx]))
