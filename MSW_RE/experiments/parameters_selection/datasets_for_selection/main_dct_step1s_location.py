...
class ImageAnnotator:
    ...
    def show_image_with_rect(self):
        ...
        # 将 500x500 转换为 display_image 上的大小
    ...
    def save_points(self, save_dir):
        ...
        with open(json_path, "w") as f:
            json.dump(self.points_list, f, indent=4)
...
if __name__ == "__main__":
    ...
    if not image_paths:
        ...
        while True:
            key = cv2.waitKey(1)
            if key == 27:  # ESC退出
                os.makedirs(os.path.join(folder_selected, "located"), exist_ok=True)
                annotator.save_points(os.path.join(folder_selected, "located"))  # 保存到原文件夹
                break
...
#     source_points_list = pickle.load(f)
#
