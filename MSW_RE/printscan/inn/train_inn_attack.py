...
def train(args):
    ...
    for epoch in range(args.epochs):
        for _, (d_t_block_out, scan_block_out) in enumerate(train_loader):
            ...
            # plt.subplot(121)
            # plt.imshow(d_t_block_out[0].squeeze(0).detach().cpu().numpy(), cmap='gray')
            # plt.subplot(122)
            # plt.imshow(scan_block_out[0].squeeze(0).detach().cpu().numpy(), cmap='gray')
            # plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ppi", type=int, default=1200)
